...
[ -f ostserver_latest_amd64.deb ] && curl --location --header "JOB-TOKEN: ${CI_JOB_TOKEN}" --upload-file ostserver_latest_amd64.deb "${PACKAGE_REGISTRY_LATEST}/ostserver_latest_amd64.deb"
...
[ -f ostserver_latest_amd64.deb ] && ASSETS="$ASSETS --assets-link {\"name\":\"ostserver_latest_amd64.deb\",\"url\":\"${PACKAGE_REGISTRY_LATEST}/ostserver_latest_amd64.deb\"}"
